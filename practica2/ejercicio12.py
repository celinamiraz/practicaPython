...
def es_borde(fila, columna):
    ...
    if (fila == 0 or fila+1 == len(minas) 
       or columna == 0 or columna+1 == len(minas[fila])):
        return True
    else:
        return False
...
def es_esquina(fila, columna):
    ...
    if ((fila == 0 or fila+1 == len(minas)) 
       and (columna == 0 or columna+1 == len(minas[fila]))):
        return True
    else:
        return False
...
def procesar_borde(fila, columna):
    """Cuenta las minas cuando la celda es parte del borde y
    no es una esquina."""
...
def buscar_minas(posicion):
    """Busca las minas de cada celda interna o llama a procesos
    para buscar minas en los bordes."""
